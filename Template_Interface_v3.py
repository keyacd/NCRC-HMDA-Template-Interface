...
class Widget_Home(Widget):
	def __init__(self, parent=None):
		super().__init__(parent)
		# Intro
		txt = br(bold("Welcome to the NCRC HMDA Template Interface!") + "(version 3.0)", "This interface exists to help simplify the process of filling out NCRC's HMDA template.")
		...
		self.add_labeled(self.data_filename, "HMDA data filename")
		self.add_wide()
		# MSA
			...
			if geo_type == "MSA":
				...
				if self.data_states == []:
					for i in self.data_other:
						...
						#self.data_tracts.append(str(i[2]))
				if geo_type == "State":
					self.update_sub(self.data_states, "State(s)")
				else: # County
					self.update_sub(self.data_counties, "Count(y/ies)")
				#else: # Tract
					#print(self.data_tracts)
					#self.update_sub(self.data_tracts, "Tract(s)")
			self.geo_name_select = []
			self.update_select()
...
class Widget_Lender(Widget):
	def __init__(self, parent=None):
		...
		self.add_wide(txt)
	def get(self):
		result = check_list(self.lenders.text().split(","))
	...
	def finish(self):
		...
		for i in geos:
			for j in self.geo.data_MSAs:
				if (geo_type == "MSA" and j[0] == i) or (geo_type == "State" and j[2] == i) or (geo_type == "County" and j[1] == i): # Census Tract not added
					MFI += int(j[3])
					MFI_count += 1
					FPL += int(j[4])
					FPL_count += 1
		...
		if lenders != []:
			row_target = 2
			self.end.reset("Getting and setting HMDA Target Data...", 0, row_max)
			for row in range(2, row_max):
				id = res_ID(template_HMDA_all.range("BN" + str(row)).value)
				for j in lenders:
					if j == id:
						template_HMDA_target.range("A" + str(row_target) + ":CH" + str(row_target)).value = template_HMDA_all.range("A" + str(row) + ":CH" + str(row)).value
						row_target += 1
				self.end.add_step()
